...
def create_required_temp_tables(cursor):
    ...
    print(f"\n📊 Creating date range retention table (active between 1/30/2025 and 10/25/2025, never medically ineligible or cancelled):")
    execute_with_timing(cursor, "DROP TEMPORARY TABLE IF EXISTS tmp_current_year_quarters_retention_users", "Drop date range retention table")
    execute_with_timing(cursor, f"""
        CREATE TEMPORARY TABLE tmp_current_year_quarters_retention_users AS
        SELECT DISTINCT
            s.user_id,
            COUNT(DISTINCT DATE(bus.created_at)) AS active_days
        FROM subscriptions s
        JOIN billable_user_statuses bus ON s.user_id = bus.user_id
        WHERE bus.partner IN ('Apple', 'Amazon')
          AND s.status = 'Active'
          AND bus.created_at BETWEEN '2025-01-30' AND '2025-10-25'
          -- Exclude users who EVER had medical-ineligible or cancelled status in the date range
          AND NOT EXISTS (
              SELECT 1 
              FROM billable_user_statuses bus2
              WHERE bus2.user_id = s.user_id
                AND bus2.created_at BETWEEN '2025-01-30' AND '2025-10-25'
                AND (bus2.user_status = 'medical_ineligible' 
                     OR bus2.subscription_status = 'Cancelled')
          )
        GROUP BY s.user_id
        HAVING active_days >= 1
    """, "Create date range retention table")
    execute_with_timing(cursor, "CREATE INDEX idx_current_year_quarters_retention_user_id ON tmp_current_year_quarters_retention_users(user_id)", "Index date range retention table")
