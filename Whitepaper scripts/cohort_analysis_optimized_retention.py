...
def create_all_health_metrics_at_once(cursor):
    ...
    execute_with_timing(cursor, """
        ...
        FROM (
            SELECT 
                ub.user_id,
                pbwv.effective_date as baseline_weight_date,
                pbwv.value * 2.20462 as baseline_weight_lbs,
                ROW_NUMBER() OVER (PARTITION BY ub.user_id ORDER BY pbwv.effective_date ASC) as rn  -- FIRST measurement
            FROM tmp_user_base_filtered ub
            JOIN body_weight_values pbwv ON ub.user_id = pbwv.user_id
            WHERE pbwv.effective_date >= DATE_SUB(ub.subscription_start_date, INTERVAL 30 DAY)  -- 30 days BEFORE
    ...
    execute_with_timing(cursor, """
        ...
        FROM (
            SELECT 
                ...
                ROW_NUMBER() OVER (PARTITION BY bw.user_id ORDER BY pbwv.effective_date DESC) as rn
            FROM tmp_cte_baseline_weight bw
            JOIN body_weight_values pbwv ON bw.user_id = pbwv.user_id
            WHERE pbwv.effective_date >= DATE_ADD(bw.baseline_weight_date, INTERVAL 180 DAY)
        ) ranked
        WHERE rn = 1
    ...
    execute_with_timing(cursor, """
        ...
        WHERE rn = 1
    """, "Create baseline BMI CTE table (30 days before to any time after - first measurement)")
    ...
    execute_with_timing(cursor, """
        ...
        WHERE rn = 1
    """, "Create latest BMI CTE table (ONE row per user)")
    ...
    execute_with_timing(cursor, """
        ...
        WHERE rn = 1
    """, "Create baseline A1C CTE table (30 days before to any time after - first measurement)")
...
def get_super_optimized_query(cohort_table: str, cohort_name: str) -> str:
    ...
    return f"""
        SELECT 
            ...
            ROUND(AVG(mhm.baseline_bmi - mhm.latest_bmi), 2) as bmi_change_units,
            ...
            ROUND(AVG(mhm.baseline_a1c - mhm.latest_a1c), 2) as a1c_change_avg,
            COUNT(CASE WHEN mhm.baseline_a1c IS NOT NULL AND mhm.latest_a1c IS NOT NULL THEN 1 END) as a1c_change_n,
            ROUND(AVG(CASE WHEN mhm.baseline_a1c >= 6.5 THEN mhm.baseline_a1c - mhm.latest_a1c END), 2) as a1c_change_6_5_plus,
            COUNT(CASE WHEN mhm.baseline_a1c >= 6.5 AND mhm.latest_a1c IS NOT NULL THEN 1 END) as a1c_change_6_5_plus_n,
            ROUND(AVG(CASE WHEN mhm.baseline_a1c >= 8.0 THEN mhm.baseline_a1c - mhm.latest_a1c END), 2) as a1c_change_8_plus,
            COUNT(CASE WHEN mhm.baseline_a1c >= 8.0 AND mhm.latest_a1c IS NOT NULL THEN 1 END) as a1c_change_8_plus_n,
            ROUND(AVG(CASE WHEN mhm.baseline_a1c >= 9.0 THEN mhm.baseline_a1c - mhm.latest_a1c END), 2) as a1c_change_9_plus,
            ...
            ROUND(AVG(CASE WHEN mhm.baseline_bp_systolic >= 130 OR mhm.baseline_bp_diastolic >= 80
                         THEN mhm.baseline_bp_systolic - mhm.latest_bp_systolic END), 2) as bp_change_130_80_plus,
            ...
            ROUND(AVG(CASE WHEN mhm.baseline_bp_systolic >= 140 OR mhm.baseline_bp_diastolic >= 90
                         THEN mhm.baseline_bp_systolic - mhm.latest_bp_systolic END), 2) as bp_change_140_90_plus,
