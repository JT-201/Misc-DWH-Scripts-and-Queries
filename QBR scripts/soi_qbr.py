...
def calculate_6month_retention(conn, df_users, partner, end_date):
    ...
    query = f"""
        ...
        JOIN partner_employers pe ON pe.user_id = bus.user_id
...
def get_clinical_data(conn, table, val_col, date_col, partner, max_date):
    query = f"""
        ...
        FROM {table} d
        JOIN partner_employers pe ON pe.user_id = d.user_id
...
def get_glp1_data(conn, partner, end_date):
    query = f"""
        ...
        JOIN partner_employers pe ON pe.user_id = p.patient_user_id
        WHERE pe.name = '{partner}'
...
def main():
    ...
    try:
        ...
        if not raw_glp.empty:
            ...
            glp1_ids = user_stats[(user_stats['total_days'] >= 90) & (user_stats['gap_pct'] <= 10.0)]['user_id'].unique()
