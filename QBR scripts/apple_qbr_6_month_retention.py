...
def create_apple_users_table(cursor, partner='Apple', end_date='2025-12-31'):
...
def create_apple_users_6month_retention_table(cursor, partner='Apple', end_date='2025-12-31'):
    """Create temporary table for 6-month retention users (ONLY for health metrics)"""
    print(f"\n🏥 Creating {partner} 6-month retention users table (for health metrics only)...")
    ...
    execute_with_timing(cursor, f"""
        CREATE TEMPORARY TABLE tmp_apple_users_base_6month AS
        SELECT DISTINCT s.user_id, s.start_date
        FROM subscriptions s
        JOIN partner_employers bus ON bus.user_id = s.user_id
        WHERE bus.name = '{partner}'
        AND s.status = 'ACTIVE'
    """, f"Create base {partner} users for 6-month retention")
    
    # Step 2: Apply 6-month retention logic (from cohort script)
    execute_with_timing(cursor, f"""
        ...
        WITH user_subscription_days AS (
            ...
            FROM tmp_apple_users_base_6month au
    ...
    execute_with_timing(cursor, "DROP TEMPORARY TABLE IF EXISTS tmp_apple_users_base_6month", "Drop base 6-month users table")
    ...
    # Print retention statistics comparison - FIX THE UNREAD RESULT ERROR
    print(f"  📊 Calculating user statistics...")
    
    # Get all users count
    cursor.execute("SELECT COUNT(*) as all_users FROM tmp_apple_users")
    all_count = cursor.fetchone()
    
    # Get 6-month retention users count
    cursor.execute("SELECT COUNT(*) as retained_users FROM tmp_apple_users_6month")
    retained_count = cursor.fetchone()
    
    print(f"  📊 All {partner} users: {all_count['all_users']}")
    print(f"  📊 6-month retention users: {retained_count['retained_users']}")
    
    # Avoid division by zero
    if all_count['all_users'] > 0:
        retention_rate = (retained_count['retained_users'] / all_count['all_users'] * 100)
...
def create_health_metrics_tables(cursor, start_date='2025-01-01', end_date='2025-12-31'):
    ...
    # Create GLP1 users table - USING 6-MONTH RETENTION USERS - FIX THE COLUMN ERROR
    ...
    execute_with_timing(cursor, f"""
        ...
        user_prescription_coverage AS (
            SELECT 
                ...
                SUM(total_prescription_days) as total_covered_days,  -- Fixed: use total_prescription_days
        ...
        AND total_covered_days >= 90   -- 60 days vs 90 days in cohort script
...
def main(partner='Apple', analysis_start_date='2025-01-01', analysis_end_date='2025-12-31', goals_start_date='2025-01-01'):
