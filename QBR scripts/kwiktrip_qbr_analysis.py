...
def create_kwik_trip_user_tables(cursor, end_date='2025-09-30'):
    """Create Kwik Trip user tables for different time periods"""
    print(f"\n📦 Creating kwik trip user tables (as of {end_date})...")
    
    
    # All users (previously 120 days)
    execute_with_timing(cursor, "DROP TEMPORARY TABLE IF EXISTS tmp_kwik_trip_users_all", "Drop Kwik Trip all users table")
    execute_with_timing(cursor, f"""
        CREATE TEMPORARY TABLE tmp_kwik_trip_users_all AS
        SELECT DISTINCT s.user_id, s.start_date
        FROM subscriptions s
        JOIN partner_employers pe ON pe.user_id = s.user_id
        WHERE pe.name = 'Kwik Trip'
          AND s.status = 'ACTIVE'
          AND s.start_date <= '{end_date}'
    """, "Create Kwik Trip all users table")

    # 180 days
    execute_with_timing(cursor, "DROP TEMPORARY TABLE IF EXISTS tmp_kwik_trip_users_180", "Drop Kwik Trip 180-day users table")
    execute_with_timing(cursor, f"""
        CREATE TEMPORARY TABLE tmp_kwik_trip_users_180 AS
        SELECT DISTINCT s.user_id, s.start_date
        FROM subscriptions s
        JOIN partner_employers pe ON pe.user_id = s.user_id
        WHERE pe.name = 'Kwik Trip'
    ...
    """, "Create Kwik Trip 180-day users table")

    # Create indexes
    for table in ['tmp_kwik_trip_users_all', 'tmp_kwik_trip_users_180']:
        execute_with_timing(cursor, f"CREATE INDEX idx_{table}_user_id ON {table}(user_id)", f"Index {table}")

def create_kwik_trip_glp1_tables(cursor, end_date='2025-10-01'):
    """Create GLP1 user tables for Kwik Trip users"""
    print(f"\n💊 Creating Kwik Trip GLP1 user tables...")

    for period in ['all', 180]:
        execute_with_timing(cursor, f"DROP TEMPORARY TABLE IF EXISTS tmp_kwik_trip_glp1_users_{period}", f"Drop Kwik Trip GLP1 {period} users table")
        execute_with_timing(cursor, f"""
            CREATE TEMPORARY TABLE tmp_kwik_trip_glp1_users_{period} AS
            WITH glp1_prescriptions AS (
                ...
                FROM tmp_kwik_trip_users_{period} au
        ...
        """, f"Create Kwik Trip GLP1 {period} users table")

        execute_with_timing(cursor, f"CREATE INDEX idx_kwik_trip_glp1_{period}_user_id ON tmp_kwik_trip_glp1_users_{period}(user_id)", f"Index Kwik Trip GLP1 {period} table")

def create_weight_metrics_tables(cursor, end_date='2025-09-30'):
    """Create weight metrics tables for Kwik Trip users using questionnaire records"""
    ...
    for period in ['all', 180]:
        ...
        execute_with_timing(cursor, f"""
            ...
            FROM (
                ...
                JOIN tmp_kwik_trip_users_{period} au ON au.user_id = qr.user_id
        ...
        execute_with_timing(cursor, f"""
            ...
            WITH ranked_weights AS (
                ...
                JOIN tmp_kwik_trip_users_{period} au ON bwv.user_id = au.user_id
...
def create_blood_pressure_tables(cursor, end_date='2025-09-30'):
    """Create blood pressure tables for Kwik Trip users"""
    ...
    for period in ['all', 180]:
        ...
        execute_with_timing(cursor, f"""
            ...
            WITH ranked_bp AS (
                ...
                JOIN tmp_kwik_trip_users_{period} au ON bpv.user_id = au.user_id
...
def create_a1c_metrics_tables(cursor, end_date='2025-09-30'):
    """Create A1C metrics tables for Kwik Trip users"""
    ...
    for period in ['all', 180]:
        ...
        execute_with_timing(cursor, f"""
            ...
            WITH ranked_a1c AS (
                ...
                FROM a1c_values av
                JOIN tmp_kwik_trip_users_{period} au ON av.user_id = au.user_id
...
def create_weight_loss_analysis(cursor):
    ...
    execute_with_timing(cursor, """
        ...
        FROM tmp_baseline_weight_all bw
        JOIN tmp_latest_weight_all lw ON bw.user_id = lw.user_id
        JOIN tmp_kwik_trip_glp1_users_all glp ON bw.user_id = glp.user_id
    ...
    execute_with_timing(cursor, """
        ...
        FROM tmp_baseline_weight_all bw
        JOIN tmp_latest_weight_all lw ON bw.user_id = lw.user_id
        LEFT JOIN tmp_kwik_trip_glp1_users_all glp ON bw.user_id = glp.user_id
    ...
    execute_with_timing(cursor, """
        ...
        FROM tmp_baseline_weight_180 bw
        JOIN tmp_latest_weight_180 lw ON bw.user_id = lw.user_id
        JOIN tmp_kwik_trip_glp1_users_180 glp ON bw.user_id = glp.user_id
    ...
    execute_with_timing(cursor, """
        ...
        FROM tmp_baseline_weight_180 bw
        JOIN tmp_latest_weight_180 lw ON bw.user_id = lw.user_id
        LEFT JOIN tmp_kwik_trip_glp1_users_180 glp ON bw.user_id = glp.user_id
        WHERE glp.user_id IS NULL
    ...
    for time_period, period in [('All Users', 'all'), ('180 Days', '180')]:
        for demo_name, demo_value, demo_field in demographics:
            ...
            execute_with_timing(cursor, f"""
                ...
                FROM tmp_baseline_weight_{period} bw
                JOIN tmp_latest_weight_{period} lw ON bw.user_id = lw.user_id
                JOIN tmp_kwik_trip_glp1_users_{period} glp ON bw.user_id = glp.user_id
                JOIN users u ON bw.user_id = u.id
                WHERE u.{demo_field} = '{demo_value}'
...
def create_blood_pressure_analysis(cursor):
    ...
    for time_period, period in [('All Users', 'all'), ('180 Days', '180')]:
        ...
        execute_with_timing(cursor, f"""
            ...
            FROM tmp_baseline_blood_pressure_{period} bbb
            JOIN tmp_latest_blood_pressure_{period} lbb ON bbb.user_id = lbb.user_id
            JOIN tmp_kwik_trip_glp1_users_{period} glp ON bbb.user_id = glp.user_id
        ...
        execute_with_timing(cursor, f"""
            ...
            FROM tmp_baseline_blood_pressure_{period} bbb
            JOIN tmp_latest_blood_pressure_{period} lbb ON bbb.user_id = lbb.user_id
            LEFT JOIN tmp_kwik_trip_glp1_users_{period} glp ON bbb.user_id = glp.user_id
            WHERE glp.user_id IS NULL
    ...
    for time_period, period in [('All Users', 'all'), ('180 Days', '180')]:
        for demo_name, demo_value, demo_field in demographics:
            ...
            execute_with_timing(cursor, f"""
                ...
                FROM tmp_baseline_a1c_{period} ba1c
                JOIN tmp_latest_a1c_{period} la1c ON ba1c.user_id = la1c.user_id
                JOIN tmp_kwik_trip_glp1_users_{period} glp ON ba1c.user_id = glp.user_id
                JOIN users u ON ba1c.user_id = u.id
                WHERE u.{demo_field} = '{demo_value}'
...
def create_a1c_analysis(cursor):
    ...
    for time_period, period in [('All Users', 'all'), ('180 Days', '180')]:
        ...
        execute_with_timing(cursor, f"""
            ...
            FROM tmp_baseline_a1c_{period} ba1c
            JOIN tmp_latest_a1c_{period} la1c ON ba1c.user_id = la1c.user_id
            LEFT JOIN tmp_kwik_trip_glp1_users_{period} glp ON ba1c.user_id = glp.user_id
            WHERE glp.user_id IS NULL
        """, f"Insert {time_period} no GLP1 users A1C analysis")

def export_results_to_excel(cursor, filename='kwik_trip_qbr_results.xlsx'):
...
def main_kwik_trip_analysis(end_date='2025-09-30'):
    """Main function to run Kwik Trip QBR analysis"""
    print(f"🚀 Starting Kwik Trip QBR Analysis (as of {end_date})")

    with connect_to_db() as conn:
        with conn.cursor() as cursor:
            try:
                # Create base tables
                create_kwik_trip_user_tables(cursor, end_date=end_date)
                create_kwik_trip_glp1_tables(cursor, end_date=end_date)
                ...
                print(f"\n✅ Kwik Trip QBR Analysis Complete!")

            finally:
                # Cleanup
                cleanup_tables = [
                    'tmp_kwik_trip_users_all', 'tmp_kwik_trip_users_180',
                    'tmp_kwik_trip_glp1_users_all', 'tmp_kwik_trip_glp1_users_180',
